...
# --- BOT TOKEN ---
TOKEN = os.getenv("BOT_TOKEN") or "yaha-apna-telegram-bot-token-dalo"
...
def menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "Edit VCF":
        update.message.reply_text("Send your VCF file to rename contacts (Lucifer01, Lucifer02...).")
    elif text == "Split VCF":
        update.message.reply_text("Send your VCF file to split into smaller files.")
    elif text == "Merge VCF":
        update.message.reply_text("Send multiple VCF files. After sending all, type /done to merge.")
    elif text == "Make VCF":
        update.message.reply_text("Send a TXT file containing phone numbers (one per line).")
...
def handle_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not mode:
        update.message.reply_text("Please select a feature first using the menu.")
        return
    ...
    # --- SPLIT VCF ---
        ...
        for f_name in files:
            update.message.reply_document(document=open(f_name, "rb"))
...
def main():
    ...
    print("Bot Running...")
    app.run_polling()  # synchronous, async hata diya
